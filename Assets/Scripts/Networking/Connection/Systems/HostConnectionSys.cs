...
public partial class HostConnectionSys : SystemBase {
    ...
    {
        hostStatus = HostStatus.InitializeServices;
    }

    protected override void OnUpdate() {
        // Debug.Log( $"Updating hosting server" );
        switch (hostStatus) {
            case HostStatus.FailedToHost: {
// #if !UNITY_SERVER
//                 Debug.Log( $"Host failed check console" );
// #endif
...
// #if !UNITY_SERVER
//                 Debug.Log( $"Host successful, players may now connect" );
// #endif
                hostStatus = HostStatus.Unknown;
...
// #if !UNITY_SERVER
//                 Debug.Log( $"Host initializing services" );
// #endif
                m_InitializeTask = UnityServices.InitializeAsync();
                hostStatus = HostStatus.Initializing;
                return;
            }
            case HostStatus.Initializing: {
                hostStatus = WaitForInitialization(m_InitializeTask, out m_SignInTask);
                return;
            }
            case HostStatus.SigningIn: {
// #if !UNITY_SERVER
//                 Debug.Log( $"Host logging in anonymously" );
// #endif
                hostStatus = WaitForSignIn(m_SignInTask, out m_AllocationTask);
                return;
            }
            case HostStatus.Allocating: {
// #if !UNITY_SERVER
//                 Debug.Log( $"Host waiting for allocation" );
// #endif
                hostStatus = WaitForAllocations(m_AllocationTask, out m_JoinCodeTask);
                return;
            }
            case HostStatus.GettingJoinCode: {
// #if !UNITY_SERVER
//                 Debug.Log( $"Host waiting for join code" );
// #endif
                hostStatus = WaitForJoin(m_JoinCodeTask, out JoinCode);
                return;
            }
            case HostStatus.GetRelayData: {
// #if !UNITY_SERVER
//                 Debug.Log( $"Host getting relay data" );
// #endif
                hostStatus = BindToHost(m_AllocationTask, out RelayServerData);
                return;
            }
